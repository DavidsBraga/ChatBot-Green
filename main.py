...
csv_loader = LoaderCSV("data/sustainable_products.csv")
...
def chatbot(llm: LLM, input_text: str, history: list, index: FAISSIndex):
    ...
    # BUILD SOURCES (replace the existing seen/print block)
    seen = set()
    sources = []
    for r in retrieved_chunks:
...
def main():
    ...
    while True:
        ...
        PRODUCT_KEYWORDS = ["recommend", "product", "buy", "shop", "suggest", "sustainable", "where can i get"]
        if any(kw in user_input.lower() for kw in PRODUCT_KEYWORDS):
            products = csv_loader.search_products(user_input, top_n=3)
            if products:
                print("\n Recommended Products:")
                for p in products:
                    print(csv_loader.format_product_card(p))
        
        # Process the query through the RAG pipeline
        response, history, sources = chatbot(llm, user_input, history, index)
